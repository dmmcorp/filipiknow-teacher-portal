...
export const httpAllSections = httpAction(async (ctx, request) => {
  const url = new URL(request.url);

  try {
    const result = await ctx.runQuery(internal.sections.getSections);
    ...
    return new Response(
      JSON.stringify({
        error: error.message || 'Error fetching sections.',
      }),
      {
        status: 500,
        headers: {
...
export const getSections = internalQuery({
  handler: async (ctx) => {
    const sections = await ctx.db.query('sections').collect();
